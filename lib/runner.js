...
export async function runBenchmark(config, opts) {
  const { prompt, model, maxBudgetUsd, repo, variants } = config
  ...
  async function runVariant(key) {
    ...
    try {
      ...
      results[key] = { label, error: err.message, durationMs: Date.now() - startTime }
      return
    }
  ...
  const reportData = { prompt, model, baseCommit, timestamp, variants: results }

  generateReport(reportData)
  await writeResultFiles(reportData, worktrees)
...
function printDryRun(config, variantKeys) {
  ...
  for (const key of variantKeys) {
    ...
    console.log(`  ${v.label}: ${files.length === 0 ? 'repo as-is' : files.join(', ')}`)
  }
}
