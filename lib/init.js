...
export async function initBenchmark({ repoPath, variants, name }) {
  ...
  console.log(`  3. Run: agent-bench run ${benchDir}/benchmark.yaml`)
}
...
function generateYaml(repoPath, variantKeys, foundFiles, headCommit) {
  const variantBlocks = variantKeys
    .map((key, i) => {
      const label = i === 0 ? 'A -- Baseline (repo as-is)' : `${String.fromCharCode(65 + i)} -- Variant ${key}`
      if (foundFiles.length === 0) {
        return `  ${key}:\n    label: "${label}"\n`
      }
      const filesBlock = foundFiles
        .map((f) => `      ${f}: ./variants/${key}/${f}`)
        .join('\n')
      return `  ${key}:\n    label: "${label}"\n    config_files:\n${filesBlock}\n`
    })
    .join('\n')
