...
export async function commitChanges(worktreePath_) {
  try {
    ...
    const { stdout: status } = await execFileAsync('git', ['-C', worktreePath_, 'status', '--porcelain'])
    ...
    await execFileAsync('git', ['-C', worktreePath_, 'commit', '-m', 'agent-bench: commit workspace changes'])
  } catch (err) {
