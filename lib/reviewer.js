...
export async function runReview(config, resultSet, opts) {
  ...
  async function reviewVariant(key) {
    ...
    try {
      ...
      process.stdout.write(`[${label}] review failed: could not read scores file: ${err.message}\n`)
      scores[key] = {
        label,
    ...
    if (!parsed) {
      process.stdout.write(`[${label}] review failed: invalid scores format\n`)
      scores[key] = {
        label,
...
export function parseReviewScores(scoresObj) {
  ...
  if (!scoresObj.scores || typeof scoresObj.scores !== 'object' || Array.isArray(scoresObj.scores)) return null
