export function parseInitArgs(args) {
  ...
  if (!opts.repoPath) throw new Error('Usage: agent-benchmark init <repo-path>')
  return opts
}
...
export function parseRunArgs(args) {
  ...
  if (!opts.configFile) throw new Error('Usage: agent-benchmark run <benchmark.yaml>')
  return opts
}
...
export function parseReviewArgs(args) {
  ...
  if (!opts.configFile)
    throw new Error('Usage: agent-benchmark review <benchmark.yaml> [<timestamp>]')
  return opts
}

export function parseCopilotReviewArgs(args) {
  const opts = {
    configFile: null,
    timestamp: null,
    dryRun: false,
  ...
  if (!opts.configFile)
    throw new Error('Usage: agent-benchmark copilot-review <benchmark.yaml> [<timestamp>]')
  return opts
}
